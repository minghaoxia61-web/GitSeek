...
The application settings page checks the repository's latest GitHub Release and links to its
download page. Fully automatic installation requires a separate Tauri updater signing key and a
stable update manifest. Add that only after the release channel and key backup owner are decided;
the updater private key must live in GitHub Secrets and an offline backup, never in this repository.
