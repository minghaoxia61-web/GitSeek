...
`hybrid-vector-v4`; it blends deterministic metadata scoring with a cached local semantic vector and
...
`hybrid-vector-v4`, while successful external ranking is identified as `hybrid-external-vector-v5`.
